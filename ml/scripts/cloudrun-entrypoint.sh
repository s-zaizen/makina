...
# The frozen GBDT model is baked into the image at /root/.makina/model.json
# (see Dockerfile cloudrun stage, COPY models/${MAKINA_MODEL_VERSION}/...).
# A missing file is non-fatal: /api/scan falls back to heuristic-only
# confidence and the API still serves all read routes.
MODEL_DEST="${MAKINA_MODEL:-/root/.makina/model.json}"
if [ -f "${MODEL_DEST}" ]; then
    echo "[entrypoint] frozen model present ($(wc -c < "${MODEL_DEST}") bytes, version=${MAKINA_MODEL_VERSION:-?})" >&2
else
    echo "[entrypoint] no model.json at ${MODEL_DEST} — heuristic-only confidence" >&2
fi
